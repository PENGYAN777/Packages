...
A = np.zeros(p.size)
for i in p.index:
    d[i] = CP.CoolProp.PropsSI('Dmass','P|gas',p[i],'Smass',s,fluidname) 
    t[i] = CP.CoolProp.PropsSI('T','P|gas',p[i],'Smass',s,fluidname) 
    Z[i] = CP.CoolProp.PropsSI('Z','P|gas',p[i],'Smass',s,fluidname) 
    g[i] = CP.CoolProp.PropsSI('fundamental_derivative_of_gas_dynamics','P|gas',p[i],'Smass',s,fluidname) 
    h[i] = CP.CoolProp.PropsSI('Hmass','Smass',s,'P|gas',p[i],fluidname)
    v[i] = math.sqrt(abs(2*(ht-h[i])))
    c[i] = CP.CoolProp.PropsSI('A','P|gas',p[i],'Smass',s,fluidname) 
    m[i] = v[i]/c[i]
